...
- Import path changed: update `github.com/grokify/mcpruntime` to `github.com/agentplexus/mcpkit/runtime`
- Package name changed: update `mcpruntime.` or `mcpkit.` references to `runtime.`
- OAuth2 server import path changed: update `oauth2server` to `github.com/agentplexus/mcpkit/oauth2`

### Changed

- Module path changed from `github.com/grokify/mcpruntime` to `github.com/agentplexus/mcpkit`
- Core runtime code moved to `runtime/` subpackage - import `github.com/agentplexus/mcpkit/runtime`
- OAuth2 server moved from `oauth2server/` to `oauth2/` - import `github.com/agentplexus/mcpkit/oauth2`
- Package references changed from `mcpkit.` to `runtime.` for runtime types

### Fixed

- Error handling for `fmt.Fprintf` in OAuth login error page
- Error handling for `resp.Body.Close()` in all test files
...
[unreleased]: https://github.com/agentplexus/mcpkit/compare/v0.3.0...HEAD
